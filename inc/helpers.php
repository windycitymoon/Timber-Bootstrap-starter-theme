 ...
 * @return object
 */
function timberboots__add_post_type_label_filter( $twig ) {
	$twig->addExtension( new Twig_Extension_StringLoader() );
	$twig->addFilter( new Twig_SimpleFilter( 'post_type_label', 'timberboots__get_post_type_label' ) );
	return $twig;
}
add_filter( 'timber/twig', 'timberboots__add_post_type_label_filter' );
 ...
 * @return array/null
 */
function timberboots__get_posts( $collection = null ) {
 ...
 * @return array
 */
function timberboots__get_posts_by_tax( $post_type = 'post', $taxonomy = null, $terms = null, $qty = null, $exclude = null ) {
	if ( !is_array( $terms ) )
		return null;
	$result = Timber::get_posts(
		array(
 ...
 * @return array
 */
function timberboots__get_posts_with_pagination( $post_type = 'post' ) {
	global $paged;
 ...
 * @return array/null
 */
function timberboots__get_image( $id ) {
	if ( strlen( $id ) ) {
 ...
 * @return array
 */
function timberboots__get_posts_block( $post_type, $qty ) {
 ...
 * @return array
 */
function timberboots__get_sidebar( $id ) {
 ...
 * @return string
 */
function timberboots__get_post_type_label( $post_type ) {
	$obj = get_post_type_object( $post_type );
	return apply_filters( 'timberboots/get_post_type_label' , $obj->labels->singular_name, $obj );
}
 ...
 * @return object
 */
function timberboots__add_post_type_labels( $posts ) {
	// Add a post type label to each post.
	for( $i = 0, $size = count( $posts ); $i < $size; ++$i ) {
		$posts[$i]->post_type_label = timberboots__get_post_type_label( $posts[$i]->post_type );
	}
	return $posts;
}
 ...
 * @return object
 */
function timberboots__get_menu( $menu ) {
